...
def main():
   ...
   # we'll use an arbitrary number to send back to Alice and Bob that fall within [1,p-2]
   fakeA = 50
   fakeB = 37

      
   s_input_Alice = sym_input(fakeB, a, p)
   s_input_Bob = sym_input(fakeA, b, p)
   
   
   sym_key_Alice = generate_key(SHA256.new(), long_to_bytes(s_input_Alice))
   m_key_Alice = generate_key(SHA256.new(), long_to_bytes(s_input_Alice))


   sym_key_Bob = generate_key(SHA256.new(), long_to_bytes(s_input_Bob))
   m_key_Bob = generate_key(SHA256.new(), long_to_bytes(s_input_Bob))
   ...
   alice_decrypted = aes_decrypt(alice_cipher, m_key_Alice, alice_iv)
   bob_decrypted = aes_decrypt(bob_cipher, m_key_Bob, bob_iv)
